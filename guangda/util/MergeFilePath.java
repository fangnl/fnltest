...
public class MergeFilePath {
    ...
    public void merge(String paths, String outPath, int index) throws Exception {
        ...
        if (fileLength <= 0 || pathFiles.size() <= 0) {
            logger.info("{}路径下无小文件或者是空文件", paths);
            return;
        }
    ...
    private void creatAndExecuteShell(String paths, String outPath, int index) throws Exception {
        ...
        command.append(PropertiesUtil.getValue("shell.spark.submit.command") + " ");
        command.append(PropertiesUtil.getValue("shell.spark.mainClass") + " ");
        command.append("--driver-memory " + PropertiesUtil.getValue("spark.driver-memory") + " ");
        command.append("--executor-memory " + PropertiesUtil.getValue("spark.executor-memory") + " ");
        command.append("--executor-cores " + PropertiesUtil.getValue("spark.executor-cores") + " ");
        command.append("--num-executors " + PropertiesUtil.getValue("spark.num-executors") + " ");
        command.append("--conf spark.default.parallelism= "+parallelism+" ");
        command.append(PropertiesUtil.getValue("shell.spark.jar.location") + " " + paths + " " + outPath);
//        String command = "source /etc/profile" + "\n" +
//                "cd /opt/spark-2.3.1/bin/" + "\n" +
//                PropertiesUtil.getValue("shell.spark.submit.command") + " " +
//                PropertiesUtil.getValue("shell.spark.mainClass") + " " +
//                "--driver-memory" + " " +
//                PropertiesUtil.getValue("spark.driver-memory") + " " +
//                "--executor-memory" + " " +
//                PropertiesUtil.getValue("spark.executor-memory") + " " +
//                "--executor-cores" + " " +
//                PropertiesUtil.getValue("spark.executor-cores") + " " +
//                "--num-executors" + " " +
//                PropertiesUtil.getValue("spark.num-executors") + " " +
//                "--conf " + "spark.default.parallelism=" + "3 " +
//                PropertiesUtil.getValue("shell.spark.jar.location") + " " + paths + " " + outPath;
        ...
        // System.out.println("index" + index + " " + shellPath);
